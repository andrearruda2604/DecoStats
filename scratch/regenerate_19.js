...
const MIN_PROB        = 90;
const MIN_ODD         = 1.03;
const MIN_GAMES       = 1; // Temporarily allow 1 to see all, then we inspect
...
async function run() {
  ...
  console.log(`Active fixtures on 19/05: ${activeFix.length}`);
  ...
  for (const f of activeFix) {
    ...
    try {
      ...
      const hc = homeHistory?.length || 0;
      const ac = awayHistory?.length || 0;
      console.log(`- ${homeName} x ${awayName} | Home games: ${hc} | Away games: ${ac}`);

      if (hc < 1 || ac < 1) continue;
      ...
      // Fetch odds from DB if possible to avoid API limits (wait, let's just fetch from API as the script does)
      const r = await fetch(`https://v3.football.api-sports.io/odds?fixture=${f.api_id}&bookmaker=${BOOKMAKER_ID}`, { headers: { 'x-apisports-key': env.VITE_API_FOOTBALL_KEY } });
      const d = await r.json();
      const oddsResp = d.response || [];
      ...
      console.error(`Error for ${homeName} x ${awayName}:`, e.message);
    }
  }
  ...
  console.log(`\nTotal generated unique opportunities: ${deduped.length}`);
  
  // Let's filter those with homeHistory.length >= 7 AND awayHistory.length >= 7
  // We can write a quick check to see how many survive this filter
  const cleanOpps = [];
  for (const o of deduped) {
    // Get homeHistory and awayHistory length for this fixture
    // Let's fetch f for o.fixture_id
    const f = activeFix.find(x => x.api_id === o.fixture_id);
    if (!f) continue;
    const leagueFilter = f.league_id;
    const { data: homeHistory } = await supabase.from('teams_history')
      .select('*').eq('team_id', f.home_team.api_id).eq('season', f.season)
  ...
  console.log(`How many survive the >= 7 filter: ${cleanOpps.length}`);
  ...
  if (cleanOpps.length > 0) {
    const { error } = await supabase.from('odd_tickets').upsert({
      ...
      mode: 'opp',
      total_odd: 0,
      matches_count: [...new Set(cleanOpps.map(o => o.fixture_id))].length,
      status: 'PENDING',
      ticket_data: {
    ...
    if (error) console.error('Error saving clean:', error.message);
    else console.log('Successfully saved clean opportunities.');
  } else {
    // If none survived, we can save empty list
    console.log('None survived, saving empty list.');
  }
}
run();
